...
class FormatCheckPrint(models.Model):
    ...
    height_date = fields.Float(string="Height Date (cm)")
    ...
    height_amount = fields.Float(string="Height Amount (cm)")
    ...
    height_partner = fields.Float(string="Height Partner (cm)")
    ...
    height_amount_letters = fields.Float(string="Height Amount in letter (cm)")
    ...
    height_negotiable = fields.Float(string="Height Not Negotiable (cm)")
    ...
    height_description = fields.Float(string="Height Description (cm)")
    ...
    height_account_move = fields.Float(string="Height Line Table Account Move (cm)")
    ...
    height_total_credit = fields.Float(string="Height Total Credit (cm)")
    ...
    height_total_debit = fields.Float(string="Height Total Debit (cm)")
    ...
    height_user_reviewing = fields.Float(string="Height User (cm)")
    ...
    height_user_validate = fields.Float(string="Height User (cm)")
    ...
    def set_done(self):
        ...
        if self.date:
            ...
            if self.type_date == 'mix1':
                ...
                date += str("""t-options="{'format': ', YYYY '}"/>""")
            date += """</strong>"""
            arch_base += """<div style="font-size: {height_date}cm; position: absolute; top: {top_date}cm; left: {left_date}cm; font-family:'cool_font'; width: {width_date}cm">
                                        {xml_date}
                                    </div>
        ...
        if self.amount:
            ...
            if self.currency_amount == 'after':
                amount+="""{currency}""".format(currency = currency_symbol)
            amount += """</strong>"""
            arch_base += """<div style="font-size: {height_amount}cm; position: absolute; top: {top_amount}cm; left: {left_amount}cm; width: {width_date}cm">
                                        {xml_amount}
                                    </div>
        ...
        if self.partner:
            ...
            if self.type_partner == 'direct':
                partner+="""<t t-esc='o.partner_id.name'/>
                                        """
            partner += """</strong>"""
            arch_base += """<div style="font-size: {height_partner}cm; position: absolute; top: {top_partner}cm; left: {left_partner}cm; width: {width_partner}cm">
                                        {xml_partner}
                                    </div>
        ...
        if self.amount_letters:
            ...
            amount_letters += """</strong>"""
            arch_base += """<div style="font-size: {height_amount_letter}cm; text-transform: capitalize; position: absolute; top: {top_amount_letters}cm; left: {left_amount_letters}cm; width:{width_amount_letters}cm; font-family:'Calibri'; letter-spacing: 0.1em">
                                        {xml_amount_letter}
                                    </div>
        ...
        if self.negotiable:
            arch_base += """<div style="font-size: {height_negotiable}cm; position: absolute; top: {top_negotiable}cm; left: {left_negotiable}cm; width: {width_negotiable}cm">
                                        <strong>NO NEGOTIABLE</strong>
                                    </div>
                                    """.format(height_negotiable=self.height_negotiable,top_negotiable=self.top_negotiable,left_negotiable=self.left_negotiable,width_negotiable=self.width_negotiable)
        ...
        if self.voucher:
            if self.description != 'none':
                ...
                description += """</strong>"""
                arch_base += """<div style="font-size: {height_description}cm; position: absolute; top: {top_description}cm; left: {left_description}cm; width: {width_description}cm">
                                        {xml_description}
                                    </div>
            ...
            if self.account_move:
                ...
                arch_base += """<t t-set="debit" t-value="debit+l.debit"/>
                                                    ...
                                                    <td style="font-size: {height_account_move}cm; width: {width_credit}cm; text-align:right;">
            ...
            if self.total_account_move:
                arch_base += """<div style="position: absolute; top: {top_total_credit}cm; left: {left_total_credit}cm; text-align:right;">
                                        <table>
                                            <tbody>
                                                <tr>
                                                    <td style="font-size: {height_total_credit}cm; width: {width_total_credit}cm">
                                                        <strong t-esc="debit"/>
                                                    </td>
                                                </tr>
                                            </tbody>
                ...
                arch_base += """<div style="position: absolute; top: {top_total_debit}cm; left: {left_total_debit}cm; text-align:right;">
                                        <table>
                                            <tbody>
                                                <tr>
                                                    <td style="font-size: {height_total_debit}cm; width: {width_total_debit}cm">
                                                        <strong t-esc="debit"/>
                                                    </td>
                                                </tr>
                                            </tbody>
            ...
            if self.set_user_reviewing:
                arch_base += """<div style="position: absolute; top: {top_user_reviewing}cm; left: {left_user_reviewing}cm; text-align:left;">
                                        <table>
                                            <tbody>
                                                <tr>
                                                    <td style="font-size: {height_user_reviewing}cm; width: {width_user_reviewing}cm">
            ...
            if self.set_user_validate:
                arch_base += """<div style="position: absolute; top: {top_user_validate}cm; left: {left_user_validate}cm; text-align:left;">
                                        <table>
                                            <tbody>
                                                <tr>
                                                    <td style="font-size: {height_user_validate}cm; width: {width_user_validate}cm">
